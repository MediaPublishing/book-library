...
import {
  ...
  type BookRecord,
  type BookReview,
  type BookSourceRecord,
  type BookSourceRating,
  type BookSourceDescription,
  type BookExternalIdentity,
  type AuthorIdentity,
...
function normalizeSourceRatings(values: unknown[]): BookSourceRating[] {
...
function normalizeSourceDescriptions(values: unknown[]): BookSourceDescription[] {
  return values.flatMap((value) => {
    const base = normalizeSourceBase(value);
...
function normalizeExternalIdentities(values: unknown[]): BookExternalIdentity[] {
  return values.flatMap((value) => {
    const base = normalizeSourceBase(value);
    if (!base || !isRecord(value)) return [];
...
function normalizeSourceBase(value: unknown): BookSourceRecord | null {
